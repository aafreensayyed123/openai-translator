document.addEventListener("DOMContentLoaded", function () {
    ...
    languageButton.addEventListener("click", function () {
        ...
        languageDropdown.style.display = languageDropdown.classList.contains("hidden") ? "none" : "block";
    });
    ...
    function setLanguageButton(lang) {
        let selectedOption = document.querySelector(.language-option[data-lang="${lang}"]);
        if (selectedOption) {
    ...
    async function translateTexts(texts, targetLang) {
        ...
        let response = await fetch(url, {
            ...
            body: JSON.stringify({
                model: "gpt-3.5-turbo",
                messages: [{ role: "system", content: "You are a professional website translator. Do not translate symbols, numbers, CSS code, or code snippets." }, { role: "user", content: prompt }]
            })
        });
